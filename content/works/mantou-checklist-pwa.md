---
home_status: "手机可用 · 支持离线"
home_title: "每天三件事，留下一张记录"
...
- 「破界行动」分钟（高质量阅读与批判性思维训练）；
- 今日训练状态；
- 日记是否完成。

其中「破界行动」从键盘输入改成了 **0–180 分钟、每 5 分钟一档的滑块**，90 分钟作为当前目标刻度。滑动时数值、周统计和公开 PNG 会一起更新；训练与日记继续用状态选择。

周度信息不再要求重复填写：系统自动汇总本周「破界行动」时长和力量训练次数。阶段、当前阅读、投资状态和本周交付物仍然保留，但被降为低频设置。
